...
interface TelegramSecurityCheck {
...
export const useTelegramSecurity = (): TelegramSecurityCheck => {
  const [state, setState] = useState<TelegramSecurityCheck>({
    isInTelegram: false,
    hasInitData: false,
      ...
      try {
        ...
        try {
          ...
          if (initData && typeof initData === 'object' && 'user' in initData) {
            const userData = initData.user as any
...
export const checkAdminStatus = async (telegramId: number): Promise<boolean> => {
  try {
