...
const PromosList = () => {
    ...
    const isPromoLimitReached = (promo) => {
    ...
    if (error) {
        return (<Alert icon={<IconInfoCircle size={16}/>} title="Ошибка" color="red">
        {error}
      </Alert>);
    }
    return (<Stack>
        ...
        {items.length === 0 ? (<Text c="dimmed" ta="center" py="xl">
            No promo codes found. Create one above to get started.
              ...
              <Stack gap="xs">
                ...
                {promo.expires_at && (<Group>
                    <Text size="sm" c="dimmed">Истекает:</Text>
                    <Text size="sm" fw={500} c={isPromoExpired(promo) ? 'red' : undefined}>
              ...
              {(isPromoExpired(promo) || isPromoLimitReached(promo)) && (<Alert icon={<IconInfoCircle size={16}/>} color={isPromoExpired(promo) ? 'red' : 'orange'} mt="md">
      ...
      <Modal opened={deleteModal.open} onClose={() => setDeleteModal({ open: false, promo: null })} title="Удалить промокод" centered styles={getTelegramSafeAreaStyles()}>
        <Stack>
          <Text>
