...
const UserManagement = () => {
    ...
    const fetchUsers = async (page = 1, query = '') => {
        ...
        try {
            ...
            const response = await http.get(`/api/users/search?${params}`);
    ...
    const UserCard = ({ user }) => (<Card padding="sm" shadow="sm" withBorder style={{ opacity: user.is_blocked ? 0.6 : 1 }}>
      <Flex justify="space-between" align="flex-start" mb="xs">
        ...
        <Menu shadow="md" width={200}>
          ...
          <Menu.Dropdown style={getTelegramSafeAreaDropdownStyles()}>
            <Menu.Item leftSection={<IconEye style={{ width: rem(14), height: rem(14) }}/>} onClick={() => openUserDetails(user)}>
              Подробно
            </Menu.Item>
            <Menu.Item leftSection={<IconEdit style={{ width: rem(14), height: rem(14) }}/>} onClick={() => {
            ...
            <Menu.Divider />
            <Menu.Item leftSection={<IconBan style={{ width: rem(14), height: rem(14) }}/>} color="orange" onClick={() => handleUserAction(user, 'block')}>
              Заблокировать
            </Menu.Item>
            <Menu.Item leftSection={<IconCheck style={{ width: rem(14), height: rem(14) }}/>} color="green" onClick={() => handleUserAction(user, 'unblock')}>
              Разблокировать
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item leftSection={<IconTrash style={{ width: rem(14), height: rem(14) }}/>} color="red" onClick={() => {
            setSelectedUser(user);
            openDelete();
        }}>
              Удалить
    ...
    return (<Stack>
      <Paper p="md" shadow="sm">
        ...
        <Group mb="md">
          <TextInput placeholder="Поиск по Telegram ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.currentTarget.value)} leftSection={<IconSearch size={16}/>} flex={1} onKeyPress={(e) => e.key === 'Enter' && handleSearch()}/>
          <Button onClick={handleSearch} loading={loading}>
            Поиск
          </Button>
        </Group>

        {loading ? (<Group justify="center" py="xl">
            <Loader />
          </Group>) : isMobile ? (
        // Mobile card view
        <>
            ...
            {totalPages > 1 && (<Group justify="center" mt="md">
      ...
      {/* Edit User Modal */}
      <UserEditModal user={selectedUser} opened={editOpened} onClose={closeEdit} onUserUpdated={() => fetchUsers(currentPage, searchQuery)}/>
    </Stack>);
};
